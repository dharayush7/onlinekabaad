...
const FeatureCard = ({ title, heading, description, link, imgSrc, alt, reverse,scale2 }) => {
  return (
    <Row
    ...
    >
      ...
      <Col className={`p-6 text-center ${reverse ? 'text-lg-end' : 'text-lg-start'}`} >
        <div className="title-sm fg-default-3 pb-2" style={{color:"#545454"}}>{title}</div>
        <h3  style={{ color: "#01A1ff", fontSize: "4rem" }}>{heading}</h3>
        <div className="fg-default-2 blacktext" >{description}</div>
        <div className="pb-3 pt-6">
          <Button href={link} variant="link" className="label-md text-decoration-none"  >
